...
(defn- swap-links [s num1 num2]
  (-> s
      (clojure.string/replace (re-pattern (str "\\[" num1 "\\]")) "[TEMP]")
      (clojure.string/replace (re-pattern (str "\\[" num2 "\\]")) (str "[" num1 "]"))
      (clojure.string/replace #"\[TEMP\]" (str "[" num2 "]"))))
...
;; => "Lorem[1] ipsum[3] dolor[2] sit[3]\n\n[1]: sharks.com\n[2]: dogs.com\n[3]: cats.com\n"
  ...
  (def steps
    (sorting-steps (distinct reflist)))

  steps
;; => [[3 2]]

  (apply-sort content steps)
